...
size_t latin_to_UTF32(const char *latin_str, uint32_t *utf32_output, size_t utf32_len) {
  ...
  for (size_t i = 0; i < utf32_len; i++) {
    ...
    (!match_system(output_endianess)) ? utf32_output[i] = swap_bytes_32((uint32_t)uchar_value) : utf32_output[i] = (uint32_t)uchar_value; 
  }
...
size_t UTF32_to_latin(const uint32_t *UTF32_str, size_t len, char *output) {
    uint32_t utf32_char;
    uint8_t latin_char;
